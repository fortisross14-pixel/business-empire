...
export function ProductsView({ world, produce, setProductPrice, setProductQuality, setProductionSetup, assignPartner, openContract, openCreator, commissionStudy, releaseProduct, retargetProduct, discardProduct, setProductArchived, openMarketing, openSegments, focusProductId, onFocusHandled }: {
  ...
  return <div>
    ...
    </div> : <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(330px,1fr))", gap: 12 }}>
      {visible.map(({ sku, si }) => {
        ...
        return <button className={`product-portfolio-card rarity-${sku.rarity}`} key={sku.id} onClick={() => setSelectedId(sku.id)} style={{ textAlign: "left", cursor: "pointer", background: sku.archived ? "linear-gradient(180deg,#fafafa,#f1f5f9)" : "linear-gradient(180deg,#fff,#f7fafc)", border: `1px solid ${C.line}`, borderRadius: UI.radius.lg, padding: 13, color: C.ink, boxShadow: UI.shadow.card, transition: "transform .12s, box-shadow .12s, border-color .12s", opacity: sku.archived ? .78 : 1 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "start", gap: 10 }}>
            <div style={{ minWidth: 0 }}><div style={{ fontSize: 15, fontWeight: 900, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{sku.name}</div><div style={{ color: C.faint, fontSize: 10.5, marginTop: 2 }}>{archetypeByKey(sku.productKey)?.label ?? sku.productKey} · V{sku.version ?? 1}</div></div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "158px minmax(0,1fr)", gap: 13, alignItems: "center", marginTop: 11 }}>
            <div style={{ display: "grid", placeItems: "center" }}><ProductVisualCard world={world} sku={sku} size={152} showLabels={false} /></div>
    ...
    {selected && <ProductDetailModal world={world} sku={selected} si={selectedIndex} onClose={() => setSelectedId(null)} produce={produce} setProductPrice={setProductPrice} setProductQuality={setProductQuality} setProductionSetup={setProductionSetup} assignPartner={assignPartner} openContract={openContract} openCreator={openCreator} commissionStudy={commissionStudy} releaseProduct={releaseProduct} retargetProduct={retargetProduct} discardProduct={discardProduct} setProductArchived={setProductArchived} openMarketing={openMarketing} openSegments={openSegments} />}
  </div>;
}

function ProductDetailModal({ world, sku, si, onClose, produce, setProductPrice, setProductQuality, setProductionSetup, assignPartner, openContract, openCreator, commissionStudy, releaseProduct, retargetProduct, discardProduct, setProductArchived, openMarketing, openSegments }: {
  ...
  return <div className="product-detail-backdrop" style={{ position: "fixed", inset: 0, zIndex: 95, background: "rgba(4,17,30,.56)", display: "grid", placeItems: "center", padding: 18 }} onMouseDown={(e) => { if (e.target === e.currentTarget) onClose(); }}>
    <div className="product-detail-card" style={{ width: "min(820px,96vw)", maxHeight: "90vh", overflow: "auto", background: "white", borderRadius: 16, border: `1px solid ${C.line}`, boxShadow: "0 24px 70px rgba(0,0,0,.28)" }}>
      ...
      <div style={{ padding: 16 }}>
        {sku.archived ? <div><SectionTitle title="Archived product" text="This SKU is outside the active operating portfolio, but its review, studies, sales, contribution and product history are preserved." /><InfoGrid rows={[["Category", archetypeByKey(sku.productKey)?.label ?? sku.productKey], ["Version", `V${sku.version ?? 1}`], ["Product review", `★ ${(sku.reviewScore ?? 1).toFixed(1)} / 5`], ["Lifetime units", fmtNum(sku.unitsSoldTotal ?? 0)], ["Lifetime contribution", fmtMoney(sku.contributionTotal ?? 0)]]} />{sku.marketStudy && <ProductStudyPanel sku={sku} />}<div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(220px,1fr))", gap: 8, marginTop: 14 }}><button style={ctrlBtn} onClick={() => { const result = setProductArchived(si, false); if (result.ok) onClose(); else setMessage(result.reason ?? "Could not restore product."); }}>↩ Restore to portfolio</button><button style={bigBtn} onClick={() => openCreator(sku.id)}>Create redesigned V{(sku.version ?? 1) + 1}</button></div><div style={{ color: C.faint, fontSize: 9.8, lineHeight: 1.45, marginTop: 8 }}>Restoring a previously launched SKU resumes its listing with zero stock; order a batch before expecting sales. A new version keeps this one archived.</div></div> : <>
        ...
        {stage !== "design" && stage !== "manufacturing" && <div style={{ marginTop: 16, borderTop: `1px solid ${C.line}`, paddingTop: 12 }}><b style={{ fontSize: 11.5 }}>Portfolio housekeeping</b><div style={{ color: C.faint, fontSize: 9.8, lineHeight: 1.45, marginTop: 3 }}>Archive a product you no longer plan to operate. Commercial history stays available and the SKU can be restored later.</div>{confirmArchive ? <div style={{ border: "1px solid #fed7aa", background: "#fffaf0", borderRadius: 10, padding: 10, marginTop: 8 }}><b style={{ color: C.amber, fontSize: 11 }}>{sku.inventory > 0 ? `Clear ${fmtNum(sku.inventory)} units and archive?` : "Archive this SKU?"}</b><div style={{ color: C.dim, fontSize: 9.8, marginTop: 3 }}>{sku.inventory > 0 ? `Clearance recovers ${fmtMoney(archiveRecovery)} (25% of inventory cost). The remainder is written off.` : "The SKU stops selling and leaves active portfolio views."}</div><div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 6, marginTop: 8 }}><button style={ctrlBtn} onClick={() => setConfirmArchive(false)}>Cancel</button><button style={{ ...ctrlBtn, color: C.amber, borderColor: "#fed7aa" }} onClick={() => { const result = setProductArchived(si, true, sku.inventory > 0); if (result.ok) onClose(); else setMessage(result.reason ?? "Could not archive product."); }}>Confirm archive</button></div></div> : <button disabled={(sku.mfgBatchSize ?? 0) > 0} title={(sku.mfgBatchSize ?? 0) > 0 ? "Wait for the inbound batch before archiving." : undefined} style={{ ...ctrlBtn, width: "100%", marginTop: 8, color: C.amber, opacity: (sku.mfgBatchSize ?? 0) > 0 ? .45 : 1 }} onClick={() => setConfirmArchive(true)}>{sku.inventory > 0 ? `Clear stock & archive · recover ${fmtMoney(archiveRecovery)}` : "Archive product"}</button>}</div>}
        </>}
