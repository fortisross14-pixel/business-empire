...
export function LineChart({ series, height = 150, fmt = (v: number) => v.toFixed(0), zeroLine = false, markers = [] }:
  ...
  const W = 520, H = height, pad = { l: 10, r: 10, t: 15, b: 21 };
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} className="game-line-chart" role="img" aria-label={`Line chart${series.some((item) => item.label) ? `: ${series.map((item) => item.label).filter(Boolean).join(", ")}` : ""}`} style={{ width: "100%", height: H, display: "block", overflow: "visible" }}>
      ...
      <text x={pad.l} y={12} fill={C.faint} fontSize="11" fontWeight="700" fontFamily="ui-monospace">{fmt(max)}</text>
      <text x={pad.l} y={H - 5} fill={C.faint} fontSize="11" fontWeight="700" fontFamily="ui-monospace">{fmt(min)}</text>
    </svg>
  );
}
...
export function DonutChart({ segments, centerLabel, centerValue, size = 210 }: { segments: { label: string; value: number; color: string }[]; centerLabel: string; centerValue: string; size?: number }) {
  ...
  return <div className="donut-wrap" role="img" aria-label={`${centerLabel}: ${centerValue}. ${segments.map((segment) => `${segment.label} ${Math.round(segment.value / total * 100)}%`).join(", ")}`} style={{ width: size, height: size, position: "relative", flex: "0 0 auto" }}>
    ...
    <div style={{ position: "absolute", inset: "23%", borderRadius: "50%", background: "linear-gradient(145deg,#fff,#edf5fb)", boxShadow: "inset 0 0 0 1px rgba(130,157,178,.18),0 4px 12px rgba(17,45,70,.12)", display: "grid", placeItems: "center", textAlign: "center", padding: 8 }}><div><small style={{ display: "block", color: C.faint, fontSize: 11, lineHeight: 1.1, fontWeight: 900, letterSpacing: .45 }}>{centerLabel}</small><b style={{ display: "block", color: C.ink, font: "900 24px ui-monospace,monospace", marginTop: 3 }}>{centerValue}</b></div></div>
  </div>;
}
...
export const Stat = ({ label, value, color = C.ink, delta }: { label: string; value: string; color?: string; delta?: number }) => (
  <div style={{ background: "linear-gradient(180deg,#ffffff 0%,#f6f9fb 100%)", border: `1px solid ${C.line}`, borderRadius: UI.radius.md, padding: "11px 13px", minWidth: 112, boxShadow: UI.shadow.low }}>
    <div style={{ color: C.dim, fontSize: 12, lineHeight: 1.2, fontWeight: 800, textTransform: "uppercase", letterSpacing: .55 }}>{label}</div>
    <div style={{ color, fontSize: 20, fontWeight: 800, fontFamily: "ui-monospace", letterSpacing: -.5 }}>{value}</div>
    {delta != null && <div style={{ color: delta >= 0 ? C.green : C.red, fontSize: 12, fontWeight: 750 }}>{delta >= 0 ? "▲" : "▼"} {(Math.abs(delta) * 100).toFixed(1)}%</div>}
  </div>
);
...
export const FieldLabel = ({ children }: { children: React.ReactNode }) => (
  <div style={{ color: C.dim, fontSize: 12.5, lineHeight: 1.25, fontWeight: 750, textTransform: "uppercase", letterSpacing: .65, marginBottom: 8 }}>{children}</div>
);
...
export const Seg = ({ label, opts, val, set }: { label: string; opts: string[]; val: string; set: (v: string) => void }) => (
  <div><div style={{ color: C.dim, fontSize: 12, lineHeight: 1.25, fontWeight: 700, marginBottom: 6 }}>{label}</div>
    <div role="group" aria-label={label} style={{ display: "flex", gap: 4, background: "#e8eef2", border: `1px solid ${C.line}`, borderRadius: UI.radius.md, padding: 4, flexWrap: "wrap" }}>
      {opts.map((o) => <button key={o} type="button" aria-pressed={val === o} onClick={() => set(o)} style={{ minHeight: 44, background: val === o ? "linear-gradient(180deg,#249eea,#147fc8)" : "transparent", color: val === o ? "#fff" : C.dim, border: "none", borderRadius: 8, padding: "9px 12px", fontSize: 13, fontWeight: 750, cursor: "pointer", boxShadow: val === o ? "0 2px 6px rgba(22,141,226,.20)" : "none" }}>{o}</button>)}
    </div>
  </div>
);
...
export const StarRating = ({ value, onChange, label, readOnly = false, size = 19 }:
  ...
  <div style={{ marginBottom: 8 }}>
    ...
    <div role={readOnly ? undefined : "group"} aria-label={!readOnly ? label ?? "Rating" : undefined} style={{ display: "flex", gap: 1, alignItems: "center", flexWrap: "wrap" }}>
      {[1, 2, 3, 4, 5].map((star) => (
        <button key={star} type="button" disabled={readOnly} onClick={() => !readOnly && onChange?.(star)}
          aria-label={`${star} star${star === 1 ? "" : "s"}`}
          aria-pressed={!readOnly ? star <= Math.round(value) : undefined}
          style={{ width: 44, height: 44, minWidth: 44, minHeight: 44, display: "grid", placeItems: "center", background: "transparent", border: "none", borderRadius: 9, padding: 0, cursor: readOnly ? "default" : "pointer", lineHeight: 1, color: star <= Math.round(value) ? C.amber : C.grid }}>
...
export const DisabledReason = ({ children }: { children?: React.ReactNode }) => {
  ...
  return <div role="status" style={{ color: C.amber, fontSize: 12, lineHeight: 1.45, marginTop: 6 }}>↳ {children}</div>;
};
